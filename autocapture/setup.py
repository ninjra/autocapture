...
def plan_setup(config_path: Path, *, profile: str = "full") -> SetupPlan:
    ...
    _set_value(data, ["ui", "overlay_citations_enabled"], True, changes)
    _set_value(data, ["overlay_tracker", "enabled"], True, changes)
    _set_value(data, ["overlay_tracker", "url_plugin", "enabled"], True, changes)
    _set_value(data, ["features", "enable_retention_prune"], True, changes)
    _set_value(data, ["features", "enable_memory_service_write_hook"], True, changes)
    _set_value(data, ["features", "enable_memory_service_read_hook"], True, changes)
    ...
    _set_value(data, ["tracking", "enable_clipboard"], True, changes)
    _set_value(data, ["capture", "always_store_fullres"], True, changes)
    _set_value(data, ["capture", "record_video"], True, changes)
    _set_value(data, ["ffmpeg", "enabled"], True, changes)
    _set_value(data, ["ffmpeg", "require_bundled"], True, changes)
    _set_value(data, ["ffmpeg", "allow_disable"], False, changes)
    _set_value(data, ["ocr", "engine"], "rapidocr", changes)
    _set_value(data, ["vision_extract", "engine"], "rapidocr", changes)
    _set_value(data, ["vision_extract", "fallback_engine"], "rapidocr", changes)
    _set_value(data, ["ocr", "device"], "cuda", changes)
    _set_value(data, ["reranker", "device"], "cuda", changes)
    _set_value(data, ["reranker", "disable_in_fullscreen"], False, changes)
    _set_value(data, ["reranker", "force_cpu_in_active"], False, changes)
    _set_value(data, ["routing", "embedding"], "local", changes)
    _set_value(data, ["routing", "reranker"], "enabled", changes)
    _set_value(data, ["routing", "table_extractor"], "local", changes)
    _set_value(data, ["presets", "active_preset"], "high_fidelity", changes)
    _set_value(data, ["table_extractor", "enabled"], True, changes)
    _set_value(data, ["memory_service", "enabled"], True, changes)
    _set_value(data, ["memory_service", "enable_rerank"], True, changes)
    _set_value(data, ["memory_service", "embedder", "provider"], "local", changes)
    _set_value(data, ["memory_service", "reranker", "provider"], "stub", changes)
