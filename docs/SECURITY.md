...
- SQLite is encrypted with SQLCipher by default (`database.encryption_enabled=true`).
...
- Host tracking DB uses SQLCipher by default (`tracking.encryption_enabled=true`).
