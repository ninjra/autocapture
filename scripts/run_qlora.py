#!/usr/bin/env python
"""QLoRA training runner (local-only, optional deps)."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run QLoRA training (local-only).")
...
def main() -> int:
    ...
    try:
        ...
        write_manifest(out_dir, manifest)
        print("Missing training dependencies (bitsandbytes required).")
        return 2
    ...
    dtype_name = str(params.get("compute_dtype", "float16"))
    compute_dtype = getattr(torch, dtype_name, torch.float16)
    quant_cfg = BitsAndBytesConfig(
        load_in_4bit=True,
        bnb_4bit_quant_type=str(params.get("quant_type", "nf4")),
        bnb_4bit_use_double_quant=bool(params.get("double_quant", True)),
